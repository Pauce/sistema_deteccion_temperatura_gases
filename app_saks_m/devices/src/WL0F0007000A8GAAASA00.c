...
bool display_7inch_can_init(void){
	return winstar_init_peripheral(&can_open, can_peripheral) == Winstar_Status_Ok;
}

void display_7inch_print(
	_display_index_7inch index_f,
	_arg_display_entry type_var,
	void *var
){
	if(var == NULL) return;
	...
	switch(type_var){
		...
		case DISPLAY_TYPE_VALUE_BUZZER:{
			uint8_t value = *(uint8_t*)var;
			(void)winstar_activation_buzzer(&can_open, (uint8_t)BUZZ_REPEAT, value);
			break;
		}
		default:
			break;
