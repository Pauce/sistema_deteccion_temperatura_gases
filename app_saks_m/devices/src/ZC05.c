...
void zc05_init(TaskHandle_t notify_task_handle)
{
    ...
    uart_drv_config_t cfg1 = {
        .base       = USART1,
        .src_clk_hz = CLOCK_GetFlexCommClkFreq(1U),
        .baud_rate  = UART1_BAUD,
        .task       = notify_task_handle,
		.notify_bit = UART_DRV_NOTIFY_BIT_1,
        .threshold  = UART1_THRESHOLD,
        .irq        = FLEXCOMM1_IRQn,
...
winsen_data_t * zc05_get_data(uint32_t notif_val)
{
	...
	winsen_status_t status = winsen_parse_frame(WINSEN_SENSOR_ZC05, data_raw, &zc05_data);
