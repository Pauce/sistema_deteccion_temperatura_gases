...
static _status_mcp960x mcp960x_read_register(uint8_t idDevice, _registersPointers reg, uint8_t len_data, i2c_rtos_t* channel_i2c);
static _status_mcp960x write_single(uint8_t idDevice, _registersPointers reg, i2c_rtos_t* channel_i2c);
static _status_mcp960x read_single(uint8_t idDevice, _registersPointers reg, uint8_t len_data, i2c_rtos_t* channel_i2c);
static _status_mcp960x write_two(uint8_t idDevice, _registersPointers reg, uint8_t len_data, i2c_rtos_t* channel_i2c);

static _status_mcp960x mcp960x_thermo_sensor_config(_mcp960x_device* device, i2c_rtos_t* channel_i2c);
static _status_mcp960x mcp960x_dev_config_register(_mcp960x_device* device, i2c_rtos_t* channel_i2c);

void mcp960x_get_default_config(
...
_status_mcp960x mcp960x_channel_i2c_init(
	i2c_rtos_t* channel_i2c,
	I2C_Type *base
){
	status_t status = i2c_rtos_init(
									&channel_i2c->master_rtos_handle,
									base,
...
_status_mcp960x mcp960x_device_init(_mcp960x_device* device, i2c_rtos_t* channel_i2c){
	...
	if(mcp960x_read_register(device->id, Device_IDRevision_register, 1, channel_i2c) != MCP960x_Success)
		return MCP960x_Error;
...
_status_mcp960x mcp960x_read_thermocouple(
	_mcp960x_device* device,
	i2c_rtos_t* channel_i2c
){
	if(device == NULL || channel_i2c == NULL)
	...
	if(mcp960x_read_register(device->id, Thermocouple_Hot_Junction_THOTR, 2, channel_i2c) != MCP960x_Success)
		return MCP960x_Error;
...
_status_mcp960x mcp960x_read_status(
	_mcp960x_device* device,
	i2c_rtos_t* channel_i2c
){
	if(device == NULL || channel_i2c == NULL)
	...
	if(mcp960x_read_register(device->id, STATUS_register, 1, channel_i2c) != MCP960x_Success)
		return MCP960x_Error;
...
static _status_mcp960x mcp960x_thermo_sensor_config(
	_mcp960x_device* device,
	i2c_rtos_t* channel_i2c
){
	...
	if(write_two(device->id, Thermocouple_Sensor_Configuration_register, 1, channel_i2c) != MCP960x_Success)
		return MCP960x_Error;

	return MCP960x_Success;
}

static _status_mcp960x mcp960x_dev_config_register(
	_mcp960x_device* device,
	i2c_rtos_t* channel_i2c
){
	uint8_t register_config =
			(device->cold_resolution << 7) |
			(device->adc_resolution << 5)  |
			(device->burst_samples << 2)   |
			device->operations;

	if(write_two(device->id, register_config, 1, channel_i2c) != MCP960x_Success)
		return MCP960x_Error;

	return MCP960x_Success;
}

static _status_mcp960x mcp960x_read_register(
	uint8_t idDevice,
	_registersPointers reg,
	uint8_t len_data,
	i2c_rtos_t* channel_i2c
){
...
static _status_mcp960x write_single(
	uint8_t idDevice,
	_registersPointers reg,
	i2c_rtos_t* channel_i2c
){
	...
	status_t status = i2c_rtos_transfer(&channel_i2c->master_rtos_handle, &channel_i2c->masterXfer);
...
static _status_mcp960x read_single(
	uint8_t idDevice,
	_registersPointers reg,
	uint8_t len_data,
	i2c_rtos_t* channel_i2c
...
static _status_mcp960x write_two(
	uint8_t idDevice,
	_registersPointers reg,
	uint8_t len_data,
	i2c_rtos_t* channel_i2c
