...
uart_drv_status_t uart_drv_init(uart_drv_handle_t *h,
	...
	status_t sdk_status = USART_Init(config->base, &usart_cfg,
			config->src_clk_hz);
...
void uart_drv_deinit(uart_drv_handle_t *h) {
	...
	USART_DisableInterrupts(h->base,
			kUSART_RxLevelInterruptEnable | kUSART_TxLevelInterruptEnable);
	USART_Deinit(h->base);
	h->initialized = false;
}
...
void uart_drv_isr_handler(uart_drv_handle_t *h) {
	...
	while (status & kUSART_RxFifoNotEmptyFlag) {
		...
		if (h->count == h->threshold) {
			if (h->task != NULL) {
				xTaskNotifyFromISR(h->task, h->notify_bit, eSetBits,
						&higher_prio_woken);
			}
		}
