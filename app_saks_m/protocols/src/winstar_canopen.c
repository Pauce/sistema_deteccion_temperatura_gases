...
static uint8_t convert_to_utf16(const uint8_t *data, uint8_t *utf16_buffer,
		uint8_t datLen);
static _winstar_status winstar_send_to_lcd(mcan_rtos_handle_t *can_open,
		char *stringVal, uint8_t lenString, uint8_t _index, uint8_t subindex);
/*********************************************************************
 * Funciones auxiliares
 *********************************************************************/
static inline uint32_t cob_id_request(uint8_t node_id) {
	return 0x600 + node_id;
}
static inline uint16_t sdo_get_index(uint16_t _index) {
	return (uint16_t) (0x2000 + (uint16_t) (_index));
}

_winstar_status winstar_init_peripheral(mcan_rtos_handle_t *can_open,
		CAN_Type *base) {

	if (mcan_rtos_init(can_open, base, (TickType_t) (MAX_WAIT_CAN_RESP_TICKS))
			!= kStatus_Success)
...
_winstar_status winstar_set_ind_value(mcan_rtos_handle_t *can_open,
		uint8_t indexIndicator, uint8_t value) {

	char stringVal = (char) (value);

	if (winstar_send_to_lcd(can_open, &stringVal, (uint8_t) (1U),
			indexIndicator, LED_INDICATOR_SUBINX) != Winstar_Status_Ok)
		return Winstar_Status_Error;

	return Winstar_Status_Ok;
}

_winstar_status winstar_set_text_float(mcan_rtos_handle_t *can_open,
		uint8_t textIndicator, float value) {
...
_winstar_status winstar_set_text_string(mcan_rtos_handle_t *can_open,
		uint8_t textIndicator, char *info) {
...
_winstar_status winstar_set_text_integer(mcan_rtos_handle_t *can_open,
		uint8_t textIndicator, uint32_t value) {
	char stringVal[MAX_LEN_LCD_DATA];
	memset(stringVal, 0x00, sizeof(stringVal));
...
_winstar_status winstar_activation_buzzer(mcan_rtos_handle_t *can_open,
		uint8_t repeat, uint8_t period) {

	uint8_t data[4] = { repeat, period, period, buzzerToggle };
	_winstar_status status;

	for (uint8_t i = 0; i < 4; i++) {
		if ((status = sdo_explicit_write(can_open, (uint8_t) (DEFAULT_NODE),
				(uint32_t) (BUZZER_INDEX), (uint8_t) (i + 1),
				(const uint8_t) (data[i]))) != Winstar_Status_Ok)
			break;
	}
...
static uint8_t sdo_get_exped_cmd(uint8_t len) {
	switch (len) {
	case 1:
		return 0x2F; // expedited, 1 byte
	case 2:
		return 0x2B; // expedited, 2 bytes
	case 3:
		return 0x27; // expedited, 3 bytes
	case 4:
		return 0x23; // expedited, 4 bytes
	default:
		return 0x21; // por defecto
	}
}
...
static _winstar_status sdo_segment_write(mcan_rtos_handle_t *can_open,
		uint8_t node_id, uint16_t _index, uint8_t subindex, const uint8_t *text,
		uint8_t length) {
	if (length == 0 || length > MAX_LEN_LCD_DATA)
		return Winstar_Status_Error;
...
static _winstar_status winstar_send_receive_msg(mcan_rtos_handle_t *can_open,
	...
	CAN_DATA_SIZE, myMsg->id, data_recv,
	CAN_DATA_SIZE);
...
//			bytes_to_send);
	...
	if (bytes_to_send <= (uint8_t) (SIZE_TO_SWITCH_MODE)) {
		status = sdo_expedited_write(can_open, DEFAULT_NODE, _index, subindex,
				(const uint8_t*) (stringVal), lenString);
	} else {
		status = sdo_segment_write(can_open, DEFAULT_NODE, _index, subindex,
				(const uint8_t*) (stringVal), lenString);
	}
